...
const AdminDrivers = () => {
  ...
  const handleToggleStatus = async () => {
    const newStatus = selectedDriver.status === 'Approved' ? 'Rejected' : 'Approved';
    const confirm = window.confirm(`Are you sure you want to ${newStatus.toLowerCase()} this driver?`);
    if (!confirm) return;

    try {
      await axios.patch(
        `http://localhost:3000/api/admin/driver-status/${selectedDriver.document_id}`,
        { status: newStatus },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      // Update the main driver list
      setDrivers(prev =>
        prev.map(d =>
          d.document_id === selectedDriver.document_id
            ? { ...d, status: newStatus }
            : d
        )
      );
      ...
      alert(`Driver ${newStatus.toLowerCase()} successfully.`);
    } catch (error) {
      console.error(error);
          ...
          <tbody>
            {drivers.map(driver => (
              <tr key={driver.document_id} className="border-t">
                ...
                <td className="px-6 py-4 text-center">
                  <span className={`px-2 py-1 rounded text-xs font-semibold ${driver.status === 'Pending'
              ...
              {(selectedDriver.status === 'Approved' || selectedDriver.status === 'Rejected') && (
                <div className="mt-4">
                  <button
                    ...
                    className={`${selectedDriver.status === 'Approved'
                        ? 'bg-red-600 hover:bg-red-700'
                        : 'bg-green-600 hover:bg-green-700'
                      } text-white px-4 py-2 rounded`}
                  >
                    {selectedDriver.status === 'Approved' ? 'Reject Driver' : 'Approve Driver'}
                  </button>
                </div>
              )}
            </div>
